...
class Timetable extends Admin_Controller
{
    ...
    public function classreport()
    {
        ...
        if ($this->form_validation->run() == true) {
            if (isset($_POST['search'])) {
                ...
                $class_id    = $this->input->post('class_id');
                $data['subjects']         = $this->subject_model->getClassSubjects($class_id);
                $section_id  = $this->input->post('section_id');
                $days        = $this->customlib->getDaysname();
                $days_record = array();
                foreach ($days as $day_key => $day_value) {
                ...
                $class_id              = $this->input->post('class_id');
                $section_id            = $this->input->post('section_id');
                $data['timetable']     = $this->subjecttimetable_model->getClassTimeTable($class_id,$section_id);
     ...
     $this->load->view('layout/header', $data);
        if($add){
                if(count($data['timetable'])>0){
                    $this->load->view('admin/timetable/classtable', $data);
                }else{
                    $this->load->view('admin/timetable/add_classtable', $data);
                }
            }else{
                $this->load->view('admin/timetable/classtable', $data);
            }
    ...
    public function getClassSubjects($class_id){
       ...
       $data['subject'] = $this->subject_model->getClassSubjects($class_id);
    ...
    public function edittable()
    {
        ...
        $feecategory             = $this->feecategory_model->get();
        $data['feecategorylist'] = $feecategory;
        $class_id                = $this->input->post('table_id');
        $data['subjects']         = $this->subject_model->getClassSubjects($class_id);
        $id                      = $this->input->post('table_id');
        $data['timetable']       = $this->subjecttimetable_model->getTimeTable($id);
        $this->load->view('layout/header', $data);
        $this->load->view('admin/timetable/edit_classtable', $data);
        $this->load->view('layout/footer', $data);
    }
    
    public function getstaffName($id){
        
         if(is_array($this->subjecttimetable_model->getstaffName($id))){
            if(count($this->subjecttimetable_model->getstaffName($id))>0){
                return $this->subjecttimetable_model->getstaffName($id)[0]->name;
            }
        }
        return '';
    }
    ...
    public function doaddtimeTable(){
        $saturday='[';
        foreach($this->input->post('subject_id_saturday') as $key=>$value){
            if(!empty($value)){

            $saturday_time_from = !empty($this->input->post('saturday_time_from')[$key])?$this->input->post('saturday_time_from')[$key]:0;
            $saturday_time_to = !empty($this->input->post('saturday_time_to')[$key])?$this->input->post('saturday_time_to')[$key]:0;
            $teacher_id_saturday = !empty($this->input->post('teacher_id_saturday')[$key])?$this->input->post('teacher_id_saturday')[$key]:0;
                $saturday .='{"subject_id":'.$value.',"time_from":"'.$saturday_time_from.'","time_to":"'.$saturday_time_to.'","teacher_id":'.$teacher_id_saturday.'},';
            }
        }
        ...
        $sunday='[';
        foreach($this->input->post('subject_id_sunday') as $key=>$value){
            if(!empty($value)){
                $sunday_time_from = !empty($this->input->post('sunday_time_from')[$key])?$this->input->post('sunday_time_from')[$key]:0;
                $sunday_time_to = !empty($this->input->post('sunday_time_to')[$key])?$this->input->post('sunday_time_to')[$key]:0;
                $teacher_id_sunday = !empty($this->input->post('teacher_id_sunday')[$key])?$this->input->post('teacher_id_sunday')[$key]:0;
                    $sunday .='{"subject_id":'.$value.',"time_from":"'.$sunday_time_from.'","time_to":"'.$sunday_time_to.'","teacher_id":'.$teacher_id_sunday.'},';
                }
        }
        $sunday = rtrim($sunday, ', ');
        $sunday.=']';
         ...
         $monday='[';
        foreach($this->input->post('subject_id_monday') as $key=>$value){
            if(!empty($value)){
                $monday_time_from = !empty($this->input->post('monday_time_from')[$key])?$this->input->post('monday_time_from')[$key]:0;
                $monday_time_to = !empty($this->input->post('monday_time_to')[$key])?$this->input->post('monday_time_to')[$key]:0;
                $teacher_id_monday = !empty($this->input->post('teacher_id_monday')[$key])?$this->input->post('teacher_id_monday')[$key]:0;
                    $monday .='{"subject_id":'.$value.',"time_from":"'.$monday_time_from.'","time_to":"'.$monday_time_to.'","teacher_id":'.$teacher_id_monday.'},';
                }
        }
        $monday = rtrim($monday, ', ');
        $monday.=']';
         ...
         $tuesday='[';
        foreach($this->input->post('subject_id_tuesday') as $key=>$value){
            if(!empty($value)){
                $tuesday_time_from = !empty($this->input->post('tuesday_time_from')[$key])?$this->input->post('tuesday_time_from')[$key]:0;
                $tuesday_time_to = !empty($this->input->post('tuesday_time_to')[$key])?$this->input->post('tuesday_time_to')[$key]:0;
                $teacher_id_tuesday = !empty($this->input->post('teacher_id_tuesday')[$key])?$this->input->post('teacher_id_tuesday')[$key]:0;
                    $tuesday .='{"subject_id":'.$value.',"time_from":"'.$tuesday_time_from.'","time_to":"'.$tuesday_time_to.'","teacher_id":'.$teacher_id_tuesday.'},';
                }
        }
        ...
        $tuesday.=']';
         ...
         $wedneday='[';
        foreach($this->input->post('subject_id_wedneday') as $key=>$value){
            if(!empty($value)){
                $wedneday_time_from = !empty($this->input->post('wedneday_time_from')[$key])?$this->input->post('wedneday_time_from')[$key]:0;
                $wedneday_time_to = !empty($this->input->post('wedneday_time_to')[$key])?$this->input->post('wedneday_time_to')[$key]:0;
                $teacher_id_wedneday = !empty($this->input->post('teacher_id_wedneday')[$key])?$this->input->post('teacher_id_wedneday')[$key]:0;
                    $wedneday .='{"subject_id":'.$value.',"time_from":"'.$wedneday_time_from.'","time_to":"'.$wedneday_time_to.'","teacher_id":'.$teacher_id_wedneday.'},';
                }
        }
        $wedneday = rtrim($wedneday, ', ');
        $wedneday.=']';

         $thursday='[';
        foreach($this->input->post('subject_id_thursday') as $key=>$value){
            if(!empty($value)){
                $thursday_time_from = !empty($this->input->post('thursday_time_from')[$key])?$this->input->post('thursday_time_from')[$key]:0;
                $thursday_time_to = !empty($this->input->post('thursday_time_to')[$key])?$this->input->post('thursday_time_to')[$key]:0;
                $teacher_id_thursday = !empty($this->input->post('teacher_id_thursday')[$key])?$this->input->post('teacher_id_thursday')[$key]:0;
                    $thursday .='{"subject_id":'.$value.',"time_from":"'.$thursday_time_from.'","time_to":"'.$thursday_time_to.'","teacher_id":'.$teacher_id_thursday.'},';
                }
        }
        ...
        $thursday.=']';
         ...
         $friday='[';
        foreach($this->input->post('subject_id_friday') as $key=>$value){
            if(!empty($value)){
                $friday_time_from = !empty($this->input->post('friday_time_from')[$key])?$this->input->post('friday_time_from')[$key]:0;
                $friday_time_to = !empty($this->input->post('friday_time_to')[$key])?$this->input->post('friday_time_to')[$key]:0;
                $teacher_id_friday = !empty($this->input->post('teacher_id_friday')[$key])?$this->input->post('teacher_id_friday')[$key]:0;
                    $friday .='{"subject_id":'.$value.',"time_from":"'.$friday_time_from.'","time_to":"'.$friday_time_to.'","teacher_id":'.$teacher_id_friday.'},';
                }
        }
        $friday = rtrim($friday, ', ');
        $friday.=']';
 
        $data=['saturday'=>$saturday,'sunday'=>$sunday,'monday'=>$monday,'tuesday'=>$tuesday,'wedneday'=>$wedneday,'thursday'=>$thursday,'friday'=>$friday,'class_id'=>$this->input->post('class_id'),'section_id'=>$this->input->post('section_id'),'session_id'=>$this->setting_model->getCurrentSession()];
        $this->subjecttimetable_model->insertDayTimeTable($data);

        $this->session->set_flashdata('msg', '<div mark="alert alert-success text-center">' . $this->lang->line('success_message') . '</div>');
        redirect('admin/timetable/classreport');
    }
    public function edittimetable(){
        ...
        $data=['saturday'=>$saturday,'sunday'=>$sunday,'monday'=>$monday,'tuesday'=>$tuesday,'wedneday'=>$wedneday,'thursday'=>$thursday,'friday'=>$friday];
        $this->subjecttimetable_model->updateDayTimeTable($data,$this->input->post('id'));
    ...
    public function getSubjectName($id){
         ...
         if(is_array($this->subjecttimetable_model->getSubjectName($id))){
            if(count($this->subjecttimetable_model->getSubjectName($id))>0){
                return $this->subjecttimetable_model->getSubjectName($id)[0]->name;
            }
        }
        return '';
    }
