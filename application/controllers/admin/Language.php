...
class Language extends Admin_Controller {
    ...
    function update($lang_id){
        ...
        $this->session->set_userdata('sub_menu', 'language/index');
        $data['title'] = 'Language List';
        $language_result = $this->language_model->get($lang_id);
        $selected_lang=$this->setting_model->get();
        $json_languages=json_decode($selected_lang[0]['languages']);
        ...
        if (!is_dir(FCPATH . "application/language/" .$language)) {
            ...
            $file_content = file(FCPATH . "application/language/" . $language . "/app_files/system_lang.php");
         }else{
            $file_content = file(FCPATH . "application/language/" . $language . "/app_files/system_lang.php");
        }
        $lang_pharses = array();
        ...
        foreach($file_content as $line) {
            if(preg_match('/\$lang\[\'(.*?)\'] = "(.*?)";/s', $line, $match)){   
                $lang_pharses[$match[1]] = $match[2] ; 
           // echo $match[2].'<br>';
            }
        }

       // return;
        $data['content']=$lang_pharses;
        $data['lang_id']=$lang_id;

        $this->load->view('layout/header', $data);
        $this->load->view('admin/language/updatelang', $data);
        $this->load->view('layout/footer', $data);
    }
    function editlang(){
        $lang_id = $this->input->post('lang_id');
        $lang_key = $this->input->post('langkey');
        $lang_value = $this->input->post('langval');
        ...
        fwrite($handle, "<?php" . "\n");
         $tr = '"';
            ...
            $data = $sta . "'" . $lang_key[$i] . "'] = " . $tr . $lang_value[$i] . $tr . ";";
            fwrite($handle, $data . "\n");
        }
        fwrite($handle, "?>" . "\n");
        $this->session->set_flashdata('msg', '<div class="alert alert-success">save Success </div>');
        redirect('admin/language/update/'.$lang_id);
